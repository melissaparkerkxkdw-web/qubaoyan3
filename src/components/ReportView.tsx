...
const ReportView: React.FC<ReportViewProps> = ({ data, userData }) => {
  ...
  // 核心判断：只有当保研率大于0时，才认为有有效数据
  const hasValidData = data.metrics && data.metrics.admissionRate > 0;
  ...
  useEffect(() => {
    ...
    // 只有当 hasValidData 为 true 时才会执行到这里
    if (data.metrics) {
    ...
    if (data.admissionTrend && data.admissionTrend.length > 0) {
    ...
    if (data.graduateDestinations && data.graduateDestinations.length > 0) {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
             <div className="lg:col-span-2 flex flex-col gap-6">
                <div className="bg-white">
                  ...
                  <div className={`bg-gray-50 rounded-2xl p-6 border-l-4 ${hasValidData ? 'border-gd-primary' : 'border-red-400'} shadow-sm`}>
                ...
                {hasValidData ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                     <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
                        <h4 className="text-sm font-bold text-gray-500 mb-4">近三年本校保研率趋势(官方)</h4>
                        <div className="h-40">
                  ...
                  <div className="bg-gray-50 rounded-2xl p-8 text-center border border-dashed border-gray-300">
                    ...
                    <p className="text-gray-500 font-bold">暂无官方保研数据</p>
                    <p className="text-xs text-gray-400 mt-1">因该学校未在官方公示名单中，无法生成数据图表</p>
                  </div>
                )}
             </div>
             ...
             <div className="bg-white rounded-2xl border border-gray-100 shadow-lg p-6 flex flex-col items-center justify-center">
                ...
                {hasValidData ? (
                  ...
                  <div className="w-full h-64 flex items-center justify-center text-gray-400 text-sm">暂无模型数据</div>
                )}
             </div>
          </div>
